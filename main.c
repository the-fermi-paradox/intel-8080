...
static inline uint8_t emu8080_set_carry(_Bool x) {
  return emu8080_set_bit(flags, FLAG_CY, x);
}

static inline uint8_t emu8080_get_carry() {
  return flags & (1 << FLAG_CY);
}
...
static inline uint8_t emu8080_lrotate(uint8_t* reg) {
  *reg = emu8080_left_rotate(*reg);
  /* set the carry bit to the LSB */
  emu8080_set_carry(*reg & LSB_8);
  return *reg;
}

static inline uint8_t emu8080_lrotate_c(uint8_t* reg) {
  uint8_t tmp = (*reg & MSB_8) >> PMSB_8;
  *reg = emu8080_left_rotate(*reg);
  emu8080_set_bit(*reg, PLSB_8, emu8080_get_carry());
  emu8080_set_carry(tmp);
  return *reg;
}

static inline uint8_t emu8080_rrotate(uint8_t* reg) {
  *reg = emu8080_right_rotate(*reg);
  /* set the carry bit to the MSB */
  emu8080_set_carry((*reg >> PMSB_8));
  return *reg;
}

static inline uint8_t emu8080_rrotate_c(uint8_t* reg) {
  uint8_t tmp = *reg & LSB_8;
  *reg = emu8080_right_rotate(*reg);
  emu8080_set_bit(*reg, PMSB_8, emu8080_get_carry());
  emu8080_set_carry(tmp);
  return *reg;
}
...
int main(int argc, char** argv)
{
  ...
  for (uint16_t pc = 0;;pc++) {
    enum OpCode opcode = emu8080_read_mem(pc);
    /* These operations are always safe */
    uint8_t data1 = emu8080_read_mem(pc);
    uint8_t data2 = emu8080_read_mem(pc + 1);
    /* TODO: Op Code switch statement */
    switch (opcode) {
    case NOP:    break;
    case LXI_B:  emu8080_assn_rp(B, data2, data1); pc += 2; break;
    ...
    case MVI_B:  emu8080_assn_reg(B, data1); pc++; break;
    case RLC:    emu8080_lrotate(A); break;
    ...
    case MVI_C:  emu8080_assn_reg(C, data1); pc++; break;
    case RRC:    emu8080_rrotate(A); break;
	
    case AHRL:   break; /* Not implemented in 8080 */
    case LXI_D:  emu8080_assn_rp(D, data2, data1); pc += 2; break;
    case STAX_D: emu8080_write_memrp(D, *A); break;
    case INX_D:  emu8080_incr_rp(D); break;
    case INR_D:  emu8080_incr_reg(D); break;
    case DCR_D:  emu8080_decr_reg(D); break;
    case MVI_D:  emu8080_assn_reg(D, data1); pc++; break;
    case RAL:    emu8080_lrotate_c(A); break;
    case RDEL:   break; /* Not implemented in 8080 */
    case DAD_D:  emu8080_add_rp(D, H); break;
    case LDAX_D: emu8080_assn_reg(A, emu8080_read_memrp(D)); break;
    case DCX_D:  emu8080_decr_rp(D); break;
    case INR_E:  emu8080_incr_reg(E); break;
    case DCR_E:  emu8080_decr_reg(E); break;
    case MVI_E:  emu8080_assn_reg(E, data1); pc++; break;
    case RAR:    emu8080_rrotate_c(A); break;
 
    case RIM:    break; /* not implemented in 8080 */
    case LXI_H:  emu8080_assn_rp(H, data2, data1); pc += 2; break;
    case SHLD:
      break;
    case INX_H:
      break;
